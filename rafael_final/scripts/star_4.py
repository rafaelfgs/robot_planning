...
def set_map():
    ...
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            ...
            sys.stdout.write("\rCreating map grid: %3.1f%%" %
    ...
    if 0 <= i_goal < resolution_map[1] and \
       ...
       grid_obst[i_goal,j_goal] != np.inf:
        ...
        while np.any(grid_f != np.inf) and grid[i_goal,j_goal,0] == np.inf:
            ...
            # Imprime o andamento atual da funcao
            sys.stdout.write("\rApllying A* algorithm: %3.1f%%" %
...
def set_next_point():
    ...
    dx_next = d_next*cos(th_next)
    dy_next = d_next*sin(th_next)

    # Caso o robo esteja no indice do ponto objetivo
    if i_curr == i_goal and j_curr == j_goal:
        x_next = copy(x_goal)
        y_next = copy(y_goal)
        dx_next, dy_next = 0.0, 0.0

    # Caso o robo esteja perto do proximo ponto
...
def star():
    ...
    #a[a==0] = 255
    #a[a>255] = 0
    #cv2.imwrite("/home/rafael/Desktop/grid_oregon_2.png",a)
    #print asdf
    ...
    while not rospy.is_shutdown():
        ...
        #print i_next, i_curr, i_goal, i_goal_old, j_next, j_curr, j_goal, j_goal_old
        #print map_size, ratio, x_curr, y_curr, x_goal, y_goal
        #print np.where(grid_obst!=np.inf)
        #print np.shape(grid_obst)
        #print i_curr,j_curr, i_goal,j_goal
        #grid_obst[i_curr,j_curr]=1
        #grid_obst[i_goal,j_goal]=2
        #print grid_obst[25:33,16:26]
        #print adsf
